...
def alphabeta_search(state):
    ...
    value, move=max_value(state, player, -1000000000, 1000000000, 0)
    return move
...
def max_value(state, player, alpha, beta, depth):
    ...
    for a in actions(state, player):
        v2, a2=min_value(a, player^1, alpha, beta, depth+1)
        if v2>v:
            v, move=v2, a
            alpha=max(alpha, v)
        if v>=beta:
            if a2 is not None:
                print("\t"*depth, a2,f"Pruned alpha={v}, beta={beta}")
                global pruned
                pruned+=1
            return v, move
    return v, move
...
def min_value(state, player, alpha, beta, depth):
    ...
    move=None
    for a in actions(state, player):
        v2, a2=max_value(a, player^1, alpha, beta, depth+1)
        if v2<v:
            v, move=v2, a
            beta=min(beta, v)
        if v<=alpha:
...
def main():
    move=alphabeta_search(grid)
    print("Optimal first move is:", move)
    print("Total nodes explored:", explored)
    print("Total nodes pruned:", pruned)
    #while not is_terminal(move):
        #move=alphabeta_search(move)
        #print("Optimal next move is:", move)
